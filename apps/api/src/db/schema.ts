import { pgTable, varchar, timestamp, integer, boolean, index, foreignKey, unique, uuid, text, jsonb, inet, check, date, uniqueIndex, numeric, char, primaryKey, pgView, pgEnum } from "drizzle-orm/pg-core"
...
export const organization_domains = pgTable("organization_domains", {
	...
	index("idx_org_domains_domain").using("btree", table.domain.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.organization_id],
			foreignColumns: [organizations.id],
...
export const organization_memberships = pgTable("organization_memberships", {
	...
	settings: jsonb().default({}).notNull(),
	role_ids: uuid().array().default([""]).notNull(),
}, (table) => [
	index("idx_memberships_org").using("btree", table.organization_id.asc().nullsLast().op("uuid_ops")),
	index("idx_memberships_role_ids").using("gin", table.role_ids.asc().nullsLast().op("array_ops")),
...
export const organization_invitations = pgTable("organization_invitations", {
	...
	index("idx_invitations_email").using("btree", table.email.asc().nullsLast().op("text_ops")),
...
export const projects = pgTable("projects", {
	...
	unique("uq_projects_org_name").on(table.organization_id, table.name),
]);
